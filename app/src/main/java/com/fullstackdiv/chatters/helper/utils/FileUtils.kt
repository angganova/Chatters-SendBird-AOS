...
object FileUtils {
        ...
        if (isKitKat && DocumentsContract.isDocumentUri(context, uri)) {
            ...
            if (isExternalStorageDocument(uri)) {
                ...
                if ("primary".equals(type, ignoreCase = true)) {
                    ...
                    value["path"] = "${Environment.getExternalStorageDirectory()} /" + split[1]
                    value["size"] = File(value["path"] as String).length()
            ...
            if (isNewGooglePhotosUri(uri)) {
                ...
                try {
                    ...
                    value!!.put("size", file.length() as Int)
                } catch (e: Exception) {
                    e.printStackTrace()
                }
            ...
            value.put("size", File(value.get("path") as String).length() as Int)
    ...
    fun saveToFile(file: File, data: String) {
        ...
        if (!tempFile.renameTo(file)) {
            throw IOException("Error to rename file to " + file.getAbsolutePath())
        }
    }
    ...
    fun loadFromFile(file: File): String {
        ...
        var read: Int
        while (reader.read(buffer, 0, buffer.size) > 0) {
